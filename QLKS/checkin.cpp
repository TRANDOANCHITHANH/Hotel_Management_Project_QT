...
//    return true;
//}
void checkin::check_2(){
    QString cccd,cmnd,phone;
     bool cccd_check=false,cmnd_check=false;
//             phone=ui->phone->text();
             cccd=ui->cccd->text();
             cmnd=ui->cccd->text();
    QRegularExpression cccdrx("^[0-9]{12}$",
                                  QRegularExpression::CaseInsensitiveOption);
 QRegularExpression cmndrx("^[0-9]{9}$",
                           QRegularExpression::CaseInsensitiveOption);
 if(ui->cccd_2->isChecked()){
        if(!cccdrx.match(cccd).hasMatch()){
                 QMessageBox::warning(this,"Không Hợp Lệ","SOS ! Số CCCD Nhập Vào Gồm 12 Chữ Số!");
                 cccd_check = true;
                 return ;
        }
 }
 if(ui->cmnd->isChecked()){
            if(!cmndrx.match(cmnd).hasMatch()){
                     QMessageBox::warning(this,"Không Hợp Lệ","SOS ! Số CMND Nhập Vào Gồm 9 Chữ Số!");
                     cmnd_check= true;
                     return ;
        }
...
//             }else
   if(ui->name->text()==""){
         ...
         QMessageBox::warning(this,"Không Hợp Lệ","Địa Chỉ Không Được Để Trống !");
          return;
     }else if(ui->days->text()==""){
         QMessageBox::warning(this,"Không Hợp Lệ","Số Ngày Không Được Để Trống !");
          return;
//      }else if(check()==false){
